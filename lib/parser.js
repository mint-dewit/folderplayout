    ...
    for (let time in playlist) {
        ...
        for (let checkTime in playlist) {
            if (time < checkTime && endTime > checkTime) {
                console.log('prio conflict:',time, endTime, checkTime);
                ...
                for (let clip of playlist[time].clips) {
                    ...
                    if (endClip.toLocaleTimeString('en-US', {hour12: false}) > checkTime) {
                        // console.log('start priocheck '+checkTime+' clips here');
                        console.log('Clips started at '+checkTime+' can be played at '+endClip+' where prio is '+clip.priority);
                        let insert = false;
                        let append = false;
                    ...
                    // console.log('start:', startClip.toLocaleTimeString('en-US', {hour12: false}), '| end:', endClip.toLocaleTimeString('en-US', {hour12: false}));
                    startClip = endClip;
                }
            }
        }
...
function addFolder(item, playlist, timing) {
    console.log('add folder '+ item.path, timing);
    let clips = [];
...
function addFile(item, playlist, timing) {
    console.log('add file ', item.path, timing);
    let clip;
