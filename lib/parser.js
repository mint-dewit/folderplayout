...
function addFolder(item, playlist, timing) {
    ...
    for (let clip of app.library)
        if (clip.name.search(item.path.toUpperCase()) === 0)
            clips.push(clip);
    
    for (let clip of clips) {
        clip.priority = item.priority;
        clip.audio = item.audio === undefined ? true : item.audio; // default to true
    }
...
function addFile(item, playlist, timing) {
    ...
    for (let mediaClip of app.library)
        if (mediaClip.name.search(item.path.toUpperCase()) === 0)
            clip = mediaClip;
    
    if (clip === undefined) return;
    clip.priority = item.priority;
    // console.log(clip.name, item.audio, item.audio === undefined ? true : item.audio);
    clip.audio = item.audio === undefined ? true : item.audio; // default to true
...
parser.execute = function(timetable, date) {
    if (!app.library || app.library.length === 0) { console.log('ERR: no media in Media Library.'); return; }
    
    var timetable = timetable ? timetable : app.timetable;
    var curDate = date ? date : new Date();
...
module.exports = function (setApp) {
    if (!setApp) return;
    app = setApp;
    return parser;
};