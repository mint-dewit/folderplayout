...
interface.add = function (clip) {
	...
	server.broadcast('QUEUE LOADED', clip);
	if (queue.length === 1) {
...
interface.play = function () {
	...
	server.broadcast('QUEUE PLAYING');
}
...
interface.stop = function () {
	...
	queue.splice(0,1)
	queueState = 'stopped';
	server.broadcast('QUEUE STOPPED');
}
...
interface.clear = function () {
	...
	queue = [];
	queueState = 'stopped';
	server.broadcast('QUEUE CLEARED');
}
...
interfacte.next = function () {
	if (queue[1] !== undefined) {
		...
		server.broadcast('QUEUE NEXT');
	} else {
		queue.splice(0, 1);
		interface.app.connection.stop(1, 20);
		queueState = 'stopped';
		interface.emit('queue-empty');
		server.broadcast('QUEUE EMPTY');
	}
}
